...
class Robot(object):
    ...
    def getNearestNeighbour(self):
        ...
        for index in self.targetIndices:
            distance = (np.sqrt(np.dot((self.goodPoints[index]-startingPoint),(self.goodPoints[index]-startingPoint))))
        ...
        for i in range(len(points)):
            if points[i]["score"] == 1:
                ...
                if points[i]["collected"] != self.lastPoints[i]["collected"]:
                    ...
                    # self.x = points[i]["x"]
                    # self.y = points[i]["y"]
                    self.targetIndices = list(filter((goodCounter2).__ne__, self.targetIndices))
                    print(str(self.targetIndices))
    ...
    def checkHeading(self):
        if time.time() - self.lastMotorMovement > 4:
...
def on_message(client, userdata, msg):
    ...
    if GAME_STATE == 2:
        if msg.topic == 'robot/state':
            ...
            print("THE ANGLE: " + str(robot.angle))
            print("THE GYRO: " + str(obj['angle']%360))
            if firstState:
